...
Expected: 신규 10건 FAIL (Edit/Write에 훅이 침묵 — `tool_name != "Bash"` 조기 반환), 기존 green
...
```
(기준 3·4 — required sections·recur 방지 — 는 포맷 불가지론이므로 무수정. 번호는 `1.–2.`/`3.`/`4.` 로 유지되 리스트 정합만 확인.)
...
**사전 확인(이미 검증됨)**: 2026-07-13 현재 트리에서 skills/*/SKILL.md + agents/*.md의 구체 `references/` 경로는 전부 실재 — 이 테스트는 green으로 출발한다(태스크 진행 중 T3~T11이 추가한 참조 포함 재확인).
...
기존 README의 유효 정보(설치 명령·스킬 카탈로그)는 보존·이전하고, stale 서술(H6 잔재·"MVP: pptx pilot"류)은 현재 인벤토리로 교체. 모든 내부 링크는 실존 경로만.
...
- [ ] **Step 2: CHANGELOG [0.2.0] 엔트리 작성** — `## [Unreleased]` 바로 아래에:
...
### Verification
- `python3 -m pytest tests/ -q` — [T14 실측 수치 기입: NN passed] (R1 baseline 67).
- markdownlint-cli2 gate run on README/CHANGELOG — [exit 0 / UNVERIFIED (engine
  unavailable) 중 실측 기입], log under `.omd/omd-dogfood/verify-runs/` (uncommitted).

### Notes
- MQ-2 (fenced-JSON verify output) deliberately NOT adopted this release — no machine
  consumer yet; revisit at R3 (plan decision 1).
- LOCAL ONLY: marketplace update + app restart required after merge (spec §7 ⑤).
```
