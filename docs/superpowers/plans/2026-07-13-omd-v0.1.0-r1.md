...
`tests/test_verify_emit.py`에 기존 패턴을 따라 3케이스 추가 (아래는 로직 명세 — 호출 헬퍼는 기존 파일 것 재사용):
...
`README.md`의 Structure 코드블록(old, :14-22 — `skills/ ...`부터 `hooks/docs_verify_emit.py ...`까지)을 다음으로 교체:
...
`## Status (2026-05-31)` 절 전체(old, 제목 포함 :32-40 — "Design: ..." 줄 전까지)를 다음으로 교체:
...
`agents/doc-builder.md`의 `<Investigation_Protocol>` 목록 끝에 추가:
...
`agents/doc-verifier.md`의 `<Investigation_Protocol>` 목록 끝에 추가:
