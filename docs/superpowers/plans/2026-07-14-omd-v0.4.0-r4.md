...
| PD-3 | **G5 증거 채널 = `.omd/<slug>/stage-evidence.log`** (pilot이 append), stop guard는 mtime이 STALE_AFTER(6h) 이내인 로그만 검사. ⚠️ 이 mtime 제외는 **HK-4("낡은 방치물을 침묵시키지 마라")의 명시적·이름 붙인 예외**다 | Stop 훅은 세션 상태 저장소가 없으므로(HK-1 선례) 파일 mtime이 "이번 세션" 근사. HK-4 예외가 정당한 이유: `.verify-pending`은 *미해소 작업* 마커라 이월분도 계속 표시해야 하지만(HK-4 원 판정), stage-evidence.log는 *해당 실행의 위임 감사 기록*이고 clearing 이벤트가 없다 — 지난 세션의 완료·중단된 run을 매 Stop마다 소급 경고하면 alert fatigue(§7 ⑥)를 G5가 재도입한다. 훅 docstring에도 이 예외를 명기 |
...
| Modify | `skills/docs-learn/SKILL.md` (Step 4·6 가드 배선 — KN-3·KN-4) | 3 |
...
| Modify | `skills/docs-pilot/SKILL.md` (Step 7 가드 배선·notepad 포인터·G5 마커·Step 7b·G6) | 3·6·7·8·9 |
...
### Task 3: wiki README 계약 갱신 + 실제 쓰기 사이트 가드 배선 — KN-2 포인터 · ascent home-directory 하한(ST-3) · 영문 슬러그 규칙 & KN-3/KN-4 배선

**Files:**
- Modify: `references/wiki/README.md`
- Modify: `skills/docs-pilot/SKILL.md` (Step 7 "Append format" 불릿 — light-channel 쓰기 사이트)
- Modify: `skills/docs-learn/SKILL.md` (Step 4 · Step 6 — heavy-channel/global-copy 쓰기 사이트)
- Modify: `tests/test_wiki_two_level.py` (pin 4개 추가)

**Interfaces:**
- Consumes: Task 1·2의 `query_helper.py` (`safe_wiki_path`·`title_to_slug` — 이 태스크가 실제 소비자를 배선한다).
- Produces: README 계약 문장 3건 + `.omd/wiki/**` 실제 쓰기 사이트 3곳(pilot Step 7, learn Step 4, learn Step 6)의 가드 호출 계약.

**⚠️ 리터럴 `$HOME` 금지 (ultracode 검증 blocking 실측):** `tests/test_wiki_two_level.py`의 기존 `test_no_absolute_paths_or_env_in_two_level_wiki_contract`가 README 본문에 `$HOME` 토큰 부재를 이미 pin하고 있다. ST-3 문구는 반드시 **"the user's home directory"** 표현으로 쓰고, 어떤 스텝에서도 README에 `$HOME` 리터럴을 넣지 말 것.

- [ ] **Step 1: 실패하는 pin 테스트 추가**

```python
# tests/test_wiki_two_level.py 에 추가 — 이 파일의 기존 `_body()` 헬퍼와 Path 상수
# (WIKI_README, ROOT)를 그대로 재사용. `import os` 추가 금지(파일에 없음 — pathlib 관례).
...
(⚠️ `_body`/`WIKI_README`/`ROOT`가 그 파일에 실재하는지 먼저 읽고 확인 — 없으면 파일의 기존 pathlib 관례로 동등하게 정의. `open()`/`os.path.join` 스타일 혼입 금지.)

- [ ] **Step 2: 실패 확인** — `python3 -m pytest tests/test_wiki_two_level.py -v` → 신규 4개 FAIL, **기존 `test_no_absolute_paths_or_env_...`는 계속 PASS 상태 확인**.
...
(b) ascent를 서술하는 **두 곳 모두**에 추가 (ultracode 검증: 한 곳만 달면 실제 구현 기준이 되는 pseudocode 쪽이 무제한으로 남는다). 문구에 `$` 환경변수 토큰 금지 — "the user's home directory"로:

(b-1) 디렉토리 layout의 `▲  discovery = ascent (…)` 줄 바로 아래:
...
(b-2) `wiki_query(category)` contract 절의 pseudocode 코드펜스 **바로 아래** 한 줄 (펜스 안에 주입하지 말 것 — 스니펫 복붙 가능성 유지):
...
- [ ] **Step 3.5: 실제 쓰기 사이트 배선 (KN-3·KN-4 소비자 — README prose만으로는 가드가 실행되지 않는다)**

  - `skills/docs-pilot/SKILL.md` Step 7의 **"Append format" 불릿** 끝에 추가:
  ...
  - `skills/docs-learn/SKILL.md` **Step 4 첫 번째 sub-불릿**(style-spec 파일 갱신) 끝에 추가:
  ...
  - `skills/docs-learn/SKILL.md` **Step 6**(local→global copy) 끝에 추가:
...
- [ ] **Step 4: 통과 확인** — `python3 -m pytest tests/test_wiki_two_level.py -v` → PASS(신규 4 + 기존 전부), 전체 suite green.
- [ ] **Step 5: Commit** — `git commit -am "docs(omd): wiki README contract + write-site guard wiring — KN-2 pointer, home-dir ascent floor (ST-3), slug rule + safe_wiki_path/title_to_slug wiring (KN-3/KN-4)"`
...
```python
# tests/test_skill_contract.py 에 추가 — 이 파일의 기존 `ROOT` Path 상수와
# `.read_text(encoding="utf-8")` 관례를 그대로 재사용 (SKILLS 상수·os import는 없다 — 추가 금지)
...
- [ ] **Step 1: 실패하는 테스트 추가** — ⚠️ ultracode 검증 실측: 기존 `run_hook`은 **`subprocess.CompletedProcess`를 반환**한다(기존 테스트 5개가 `.stdout`/`.returncode`로 접근). 신규 테스트도 `.stdout`으로 읽는다. 또한 파일에 `import os`가 없으면 추가(`os.utime`은 pathlib로 대체 불가). `tmp_path`/unittest 스타일은 기존 파일 관례를 먼저 읽고 맞출 것.
...
def test_stage_evidence_gap_reported(tmp_path):
    ...
    out = run_hook({"cwd": str(tmp_path)}).stdout
...
def test_stage_evidence_old_log_ignored(tmp_path):
    """PD-3: an old log is a finished run's audit trail, not unresolved work —
    the named HK-4 exception."""
...
```python
# tests/test_output_layout_contract.py 에 추가 — ⚠️ 이 파일은 카드 본문을 이미 읽어 둔
# `CARD` 상수(경로 아님, 텍스트)를 쓰고 pathlib만 import 한다 (LAYOUT/ROOT/os 없음 — 추가 금지)
...
- [ ] **Step 4: 소비자 1줄씩** — 각 파일의 빌드/수정 절차 서술부에:
  ...
  - `skills/docs-pilot/SKILL.md` Step 8의 **`⚠️ outputs/<slug>/current.<ext> … excluded from aggregation and deletion` 제외 불릿** 끝(⚠️ "tally"라는 단어는 pilot 본문에 없다 — output-layout §5.3 어휘): `Anything inside \`outputs/<slug>/current/\` is never a cleanup target; rebuild-time overwrites go through the ownership guard (output-layout §3.4).`
...
**Interfaces:**
- Consumes: `.omd/learned.md` OBS 블록 포맷 (learning-protocol §2 — 이미 존재). wiki 쓰기 가드 배선은 **Task 3에서 완료** — 이 태스크의 Step들은 `.omd/learned.md`만 쓴다.
...
```python
# tests/test_wiki_two_level.py 에 추가 — 기존 pathlib 관례 재사용 (os import 금지)
...
- **Spec coverage**: §5 R4행의 G2·G3·G5·G6·capture·KN-2·KN-3·ST-3·KN-4 전부 태스크 매핑(1~9) + Release(10). 흡수분 ST-1은 Task 6 원자쓰기에 반영. G1-chk 재진입 가드는 기존 코드 유지(Task 7이 손대지 않음). PD-3의 mtime 컷오프는 HK-4의 명시적·이름 붙인 예외(사유는 PD-3 표와 훅 docstring에 기록).
- **Type consistency**: `tokenize/match/safe_wiki_path/title_to_slug` 시그니처가 Task 1·2·3에서 동일 — Task 3이 실제 쓰기 사이트(pilot Step 7·learn Step 4·learn Step 6)에 배선하고, Task 9는 learned.md만 쓴다. `STALE_AFTER` 재사용(Task 7). manifest 스키마는 §3.3 기존 정의 참조.
- **Placeholder scan**: 통과 — 모든 코드 스텝에 실제 코드, 모든 계약 문구에 실제 문장.
- **ultracode 검증 반영 (2026-07-14, wf_11d8e1d6)**: 확정 13건 전량 반영 — blocking 3(KN-3/KN-4 쓰기 사이트 미배선 → Task 3 확장 / `$HOME` 리터럴 pin 충돌 → home-directory 문구) · major 8(HK-4 예외 명문화, run_hook CompletedProcess, CARD/ROOT/pathlib 하니스 정합, ascent 이중 서술) · minor 2(pathlib idiom, pilot Step 8 anchor).
