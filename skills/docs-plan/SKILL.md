...
4. Save the planner's returned block to `.omd/<slug>/outline.md` — the **controller** writes this file; `doc-planner` stays read-only. Render it: `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/omd_outline_view.py .omd/<slug>/outline.md -o .omd/<slug>/gate1.html`. A non-zero exit means the renderer found gaps to report, not that the render failed — write the file and print the gap lines regardless. Surface it opportunistically — as an artifact where the harness supports one, otherwise the file path (no viewer available is a graceful degrade, never an error). Report the gaps verbatim from the renderer's output. `GAPS=0` means nothing mechanical is missing — it is **not** a judgment that the structure is good, and must never be presented as one. Present the outline + rendered sheet + gap report as Gate 1 → approval.
...
6c. **two-way split save**: save `plan.md` (RALPLAN-DR+ADR, decision process) and the outline to `.omd/<slug>/outline.md` (Final single arc, decision result) — the **controller** writes both; `doc-planner` stays read-only. Render: `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/omd_outline_view.py .omd/<slug>/outline.md -o .omd/<slug>/gate1.html`. A non-zero exit means the renderer found gaps to report, not that the render failed — write the file and print the gap lines regardless. Surfaced opportunistically — as an artifact where the harness supports one, otherwise the file path (no viewer available is a graceful degrade, never an error). Report the gaps verbatim. `GAPS=0` means nothing mechanical is missing — it is **not** a judgment that the structure is good, and must never be presented as one. Present plan.md + outline + rendered sheet + gap report as Gate 1.
</Steps>
