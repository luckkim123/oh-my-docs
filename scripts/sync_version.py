"""Version SSOT drift checker across the 4 surfaces (ported from oh-my-scholar's
R3 #15 checker; see that repo's `scripts/sync_version.py` for the original).
...
card at `<OMHA_ROOT>/cards/omd.json` (a foreign repo — optional surface).
...
def gather(repo_root) -> dict:
    ...
    card = None
    try:
        data = json.loads((omha_root / "cards" / "omd.json").read_text(encoding="utf-8"))
